...
import { installTauriControlWebSocket } from './dsh/lib/tauri-websocket'
import App from './App.tsx'
import { DirectoryProvider, FullscreenProvider, SessionProvider } from './contexts'
import { DshGate } from './features/connect/DshGate'
import { themeStore } from './store/themeStore'
import { isTauri } from './utils/tauri'
import { globalErrorHandler } from './utils/errorHandling'
import { migrateLegacySoundsDb, migrateLegacyStorageKeys } from './utils/legacyStorageMigration'
...
// OpenCodeUI → DSH Remote 旧存储迁移：必须先于任何 store 读取 localStorage
migrateLegacyStorageKeys()
void migrateLegacySoundsDb()
...
// DSH Remote：WebView 的 Origin（tauri://localhost / http://tauri.localhost）会被
// Server 的控制通道白名单拒绝（WebSocket 4010 "origin not allowed"）。将
// /ws/v1/connect 改走 Rust bridge WebSocket（原生拨号，不带 Origin 头），
// 其余 WebSocket 保持原生实现。非 Tauri 环境是 no-op。
