...
import { AccountRequiredError, ServerSessionManager } from '../session-manager'
...
describe('Server credential session', () => {
  ...
  it('requires a fresh account login when the server rejects the refresh (wiped registration)', async () => {
    const save = vi.fn(async (_credentials: DeviceCredentials) => {})
    const fetchImplementation = vi.fn<typeof fetch>(async () => {
      await Promise.resolve()
      return jsonResponse(
        { error: { code: 'AUTH_INVALID', message: 'device registration no longer exists', retryable: false } },
        401,
      )
    })
    const manager = new ServerSessionManager(
      { load: async () => storedCredentials(), save },
      (baseUrl, accessToken) => new RemoteServerApi(baseUrl, accessToken, fetchImplementation),
    )
