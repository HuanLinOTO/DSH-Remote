// DSH Remote Server REST client — ported from apps/android/src/services/api.ts.
// Web adaptation: client version comes from the app version define; no Expo imports.

import { APP_VERSION } from '../../env'
import { RemoteApiError } from '../lib/errors'
import { resolveFetch } from '../lib/tauri-fetch'
import { normalizeServerUrl } from '../lib/server-url'
import type { DeviceCredentials, DeviceIdentity, DevicePresence, RemoteDevice } from '../types'
...
/** 一次性的扫码/浏览器确认登录会话（服务端默认 600s 内有效）。 */
...
export type QrLoginPoll =
  ...
  | { status: 'complete'; token: string; account: string }
...
export class RemoteServerApi {
  ...
  private resolvedFetch?: Promise<FetchImplementation>
  ...
  private getFetch(): Promise<FetchImplementation> {
    if (this.fetchImplementation !== undefined) return Promise.resolve(this.fetchImplementation)
    this.resolvedFetch ??= resolveFetch()
  ...
  /**
   * 创建扫码/浏览器确认登录会话（原项目桌面客户端通道）：不依赖自定义 scheme
   * 回跳，客户端轮询领取结果，规避 dshremote:// 与其他应用的重叠冲突。
   */
  async startQrLogin(provider: 'zhihu' | 'github'): Promise<QrLoginSession> {
    const body = await this.request<unknown>(`/api/v1/auth/oauth/qr/start?provider=${encodeURIComponent(provider)}`, {
      method: 'POST',
      body: JSON.stringify({}),
    }, false)
    if (!isRecord(body)
      || typeof body.qrId !== 'string' || body.qrId.length < 20
      || typeof body.scanUrl !== 'string'
      || typeof body.expiresIn !== 'number' || !Number.isSafeInteger(body.expiresIn)) {
      invalidResponse('QR login session')
    }
    let scanUrl: URL
    try {
      scanUrl = new URL(body.scanUrl as string)
    } catch {
      throw new RemoteApiError('INVALID_MESSAGE', 'The server returned an invalid QR login URL.')
    }
    if (scanUrl.origin !== this.baseUrl) {
      throw new RemoteApiError('INVALID_MESSAGE', 'The server returned an untrusted QR login URL.')
    }
    return { qrId: body.qrId, scanUrl: body.scanUrl, expiresIn: body.expiresIn as number }
  }

  /** pending → 继续轮询；complete 携带一次性账号 token（仅登录用，不落盘）。 */
  async pollQrLogin(qrId: string): Promise<QrLoginPoll> {
    const body = await this.request<unknown>(`/api/v1/auth/oauth/qr/${encodeURIComponent(qrId)}`, {}, false)
    if (!isRecord(body)) invalidResponse('QR login status')
    if (body.status === 'pending' || body.status === 'expired') return { status: body.status }
    if (body.status !== 'complete' || typeof body.token !== 'string' || body.token.length < 16) {
      invalidResponse('QR login status')
    }
    // 对齐原 vscode 客户端：complete 后立即核验 token 并取账号资料。
    const profile = await this.accountMe(body.token)
  ...
  /** Register this web client under the account token. */
    ...
    const body = await this.request<unknown>('/api/v1/devices/register', {
      ...
      body: JSON.stringify({
        ...
        device: {
          ...
          clientVersion: DSH_WEB_CLIENT_VERSION,
        },
      }),
    }, false, accountToken)
  ...
  private async request<TResult>(
    ...
    try {
      response = await (await this.getFetch())(`${this.baseUrl}${path}`, {
        ...init,
...
/** Keep Server device metadata aligned with the built web bundle. */
export const DSH_WEB_CLIENT_VERSION = `web-${APP_VERSION}`
...
function mapStatus(status: number, _detail: string, serverCode: unknown): string {
