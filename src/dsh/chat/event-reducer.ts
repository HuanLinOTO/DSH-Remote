...
/** Payload types the reducer actually folds into chat items. */
const HANDLED_MUX_TYPES = new Set<string>([
  'session/event',
  'approval/requested',
  'approval/resolved',
  'question/requested',
...
export function applyMuxFrameToMessages(
  ...
  // 高频遥测帧（session/projection 等）reducer 不消费；必须原引用返回，否则每帧
  // 都会克隆整棵 chatItems 并触发 bridge + React 全量重渲染（重会话下会活锁主线程）。
  if (!HANDLED_MUX_TYPES.has(frame.payload.type)) return current
  const sessionId = stringValue(frame.payload.sessionId)
  if (sessionId === undefined) return current
  const next = applyMuxFrame(current[sessionId] ?? [], frame)
  if (next === (current[sessionId] ?? [])) return current
  return {
    ...current,
    [sessionId]: next,
  }
...
function applyNativeEvent(
  ...
  if (event.type === 'user/message') return addUserMessage(current, data, sessionId, event)
  ...
  if (event.type === 'tool/call') return applyToolCall(current, data, sessionId, 'running', view, event)
  if (event.type === 'tool/result') return applyToolCall(current, data, sessionId, 'finished', view, event)
  return current
}

function addUserMessage(current: ChatItem[], data: UnknownRecord, sessionId: string, event: NativeSessionEvent): ChatItem[] {
  ...
  const message: ChatMessage = {
    kind: 'message', id, sessionId, role: 'user', text, createdAt: eventTime(event, data),
...
function addAssistantMessage(current: ChatItem[], data: UnknownRecord, sessionId: string, event: NativeSessionEvent): ChatItem[] {
  ...
  const message: ChatMessage = {
    kind: 'message', id, sessionId, role: 'assistant', text: finalText, createdAt: eventTime(event, data),
...
function applyAssistantChunk(
  ...
  if (index < 0) {
    ...
    return [...current, {
      ...
      createdAt: eventTime(event, data),
...
export function hasVisibleMessageText(text: string): boolean {
  // eslint-disable-next-line no-misleading-character-class -- vendored regex from upstream ds-harness-remote
...
function applyToolCall(
  ...
  const next: ToolActivity = {
    ...
    createdAt: event ? eventTime(event, data) : Date.now(),
  }
  const existing = current.find(item => item.id === id)
...
/**
 * 事件时间戳：host 在事件级记录 time（data 里没有）；缺失时才回退本地时钟。
 * 回退会让整个历史折叠得到相同 createdAt，回合时长恒为 0。
 */
function eventTime(event: NativeSessionEvent, data: UnknownRecord): number {
