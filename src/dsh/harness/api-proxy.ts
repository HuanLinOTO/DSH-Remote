// Remote ApiProxy tunnel client — ported from apps/android/src/services/api-proxy.ts.
// Drives the Host's ApiProxy bridge through the four-tunnel contract
// (`harness.api.call/respond/stream.open/stream.close`, rc.2) and keeps the
// native rpcId correlation semantics. Only allowlisted Harness methods are used.

import { liftSessionProjectionFields, type RemoteClientCore } from '../vendor/client-core'
import {
...
export class RemoteApiProxy {
  ...
  async sessionList(): Promise<RemoteSession[]> {
    ...
    return Array.isArray(result.items)
      // DSH Host ≥0.1.2 rows carry title/agentPreset inside projections.values.
      ? result.items.map(item => ({ ...item, ...liftSessionProjectionFields(item as unknown as Record<string, unknown>) }))
      : []
  }
