import { RemoteGatewayError, RemoteTypertGateway, createRemoteId, type RemoteGatewayResult, type RemoteGatewayStream } from './remote-gateway'
import type { RemoteClientCore } from './index'
...
export class HarnessAlphaClient {
  ...
  async sessionRename(sessionId: string, title: string): Promise<{ title: string }> {
    const result = await this.callValue<{ title: string; seq: number }>('session/rename', {
      request: { sessionId, title },
    })
    if (typeof result.title !== 'string') {
      throw new RemoteGatewayError('INVALID_MESSAGE', 'The Host returned an invalid session rename result.')
    }
...
/**
 * DSH Host ≥0.1.2 session rows carry `title`/`agentPreset` only inside
 * `projections.values` (top-level fields do not exist on the wire); lift them
 * into the flat legacy shape so UI consumers keep working. Top-level values,
 * when an older Host provides them, win.
 */
export function liftSessionProjectionFields(value: Record<string, unknown>): Partial<HarnessRemoteSession> {
  const projections = value.projections
  const values = isRecord(projections) && isRecord(projections.values) ? projections.values : undefined
  if (values === undefined) return {}
  const patch: Partial<HarnessRemoteSession> = {}
  if (typeof value.title !== 'string' && typeof values.title === 'string') patch.title = values.title
  if (typeof value.agentPreset !== 'string' && typeof values.agentPreset === 'string') patch.agentPreset = values.agentPreset
  return patch
}
