...
export class RemoteTypertGateway {
  ...
  async open(endpoint: string, payload: unknown, signal?: AbortSignal): Promise<RemoteGatewayStream> {
    ...
    try {
      ...
      console.debug(`[dsh-debug] stream.open ok endpoint=${endpoint} streamId=${streamId}`)
    } catch (error) {
      console.debug(`[dsh-debug] stream.open FAILED endpoint=${endpoint} code=${(error as { code?: string }).code} msg=${error instanceof Error ? error.message : String(error)}`)
