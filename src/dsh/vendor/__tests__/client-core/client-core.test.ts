...
import { createEvent, createRpcError, createRpcResponse, encodeMessage } from '../../protocol'
import { BaseTransport } from '../../webrtc'
import {
  HarnessAlphaClient,
  RemoteClientCore,
  RemoteClientError,
...
class ScriptedCore {
  ...
  /** Per-endpoint scripted `harness.remote.call` results (checked before the defaults). */
...
describe('HarnessAlphaClient', () => {
  ...
  it('lifts title/agentPreset from projections.values in session/list rows', async () => {
    ...
    core.responses.set('session/list', {
      ...
      value: {
        items: [
          {
            sessionId: 'session-3a98c125-0000-0000-0000-000000000000',
            updatedAt: 5,
            running: false,
            blank: false,
            cwd: 'D:\\Projects\\DSH-Remote',
            projections: { asOfSeq: 9, values: { title: '对话命名与工作区功能问题', agentPreset: 'general' } },
          },
          {
            ...
            running: true,
            blank: false,
            title: 'flat title wins',
            projections: { asOfSeq: 1, values: { title: 'projected loses' } },
          },
        ],
      },
    })
    ...
    expect(sessions[1]?.title).toBe('flat title wins')
    await client.close()
  })

  it('emits session/renamed when the control stream projects a new title', async () => {
    const core = new ScriptedCore()
    const frames: Array<{ rpcId: string; payload: Record<string, unknown> }> = []
    const client = new HarnessAlphaClient(core as unknown as RemoteClientCore, {}, frame => frames.push(frame))
    ...
    core.emit({
      ...
      data: {
        ...
        hasValue: true,
        value: { type: 'projection', sessionId: 'session-1', key: 'title', value: '新的标题', seq: 42 },
      },
    })
    ...
    await vi.waitFor(() => {
      expect(frames).toContainEqual({
        rpcId: '',
        payload: { type: 'session/renamed', sessionId: 'session-1', title: '新的标题' },
      })
    })
    await client.close()
  })
  ...
  it('renames sessions through session/rename and returns the host title', async () => {
    ...
    await expect(client.sessionRename('session-1', '  Renamed  ')).resolves.toEqual({ title: 'Renamed' })
    expect(core.rpcCalls).toContainEqual({
      method: 'harness.remote.call',
