...
import {
  ...
  RtcChunkCodec,
} from '../../webrtc/rtc-chunking'
import { RtcDataChannelTransport } from '../../webrtc/rtc-data-channel'
import type {
  RtcDataChannel,
  RtcIceCandidateInit,
  RtcPeerConnection,
...
describe('RtcDataChannelTransport chunking integration', () => {
  it('splits a 48 KiB send into multiple wire frames and reassembles the reply', async () => {
    ...
    for (const frame of channel.sent) channel.receive(frame.buffer as ArrayBuffer)
  ...
  it('reassembles a chunked inbound 128 KiB message into one onMessage callback', async () => {
    ...
    const codec = new RtcChunkCodec()
    for (const frame of codec.encode(original)) channel.receive(frame.buffer as ArrayBuffer)
    expect(received).toHaveLength(1)
    expect(received[0]).toEqual(original)
  ...
  it('closes the transport without delivery after a malformed inbound chunk', async () => {
    ...
    channel.receive(chunkFrame(1, 2, 1).buffer as ArrayBuffer)
    await flush()
