// Connection orchestration — ported from apps/android/src/services/connection.ts.
// AdaptiveTransport (Control WSS + WebRTC/Relay) → SecureTransport (Noise IK) →
// RemoteClientCore (35s RPC timeout) → feature probe → ApiProxy (rc.2) / alpha Gateway.

import { HarnessAlphaClient, RemoteClientCore, probeRemoteHostFeatures } from '../vendor/client-core'
import { AdaptiveTransport, type RtcIceServer } from '../vendor/webrtc'
import { websocketUrl } from '../lib/server-url'
import type { DeviceIdentity, MuxStreamFrame, RemoteDevice } from '../types'
import { RemoteApiProxy } from '../harness/api-proxy'
import { SecureTransport } from './secure-transport'

export type MuxFrameHandler = (frame: MuxStreamFrame) => void
export type CloseHandler = () => void
export type RemoteHarnessClient = RemoteApiProxy | HarnessAlphaClient
...
export class DshRemoteConnection {
  ...
  requireProxy(): RemoteHarnessClient {
    if (this.proxy === undefined) throw new Error('Connect to a Host first.')
    return this.proxy
  }
