...
const LOCAL_FILE_LINK_PREFIX = '#dsh-local-file:'
...
function createStreamingHtmlDocument(resizeId: string, theme: 'light' | 'dark'): string {
  const themeHead = `<style id="dsh-html-theme">${buildHtmlSandboxThemeCss(theme, 'hidden')}</style>`
  ...
  (() => {
    ...
    const measure = () => dispatchEvent(new Event('dsh-html-measure'));
    let scheduledScripts = 0;
    let scriptQueue = Promise.resolve();
    const applyTheme = theme => {
      document.documentElement.style.colorScheme = theme;
      document.documentElement.dataset.theme = theme;
      const style = document.getElementById('dsh-html-theme');
      if (style) style.textContent = themeCss[theme] || themeCss.light;
      dispatchEvent(new CustomEvent('dsh-theme-change', { detail: { theme } }));
    ...
    const clean = (doc, scriptCount) => {
      ...
      Array.from(doc.querySelectorAll('script')).forEach((node, index) => {
        ...
        node.setAttribute('data-dsh-script-index', String(index));
        node.setAttribute('type', 'application/x-dsh-pending');
      });
    ...
    const compatible = (current, next) => {
      ...
      if (current.nodeName === 'SCRIPT') {
        return current.getAttribute('data-dsh-script-index') === next.getAttribute('data-dsh-script-index');
      }
      return true;
    };
    const patch = (current, next) => {
      ...
      if (currentElement.nodeName === 'SCRIPT' && currentElement.hasAttribute('data-dsh-script-executed')) return;
    ...
    const patchHead = next => {
      const currentNodes = Array.from(document.head.querySelectorAll('[data-dsh-stream-head]'));
      ...
      for (let index = 0; index < Math.max(currentNodes.length, nextNodes.length); index += 1) {
        ...
        if (!candidate) current?.remove();
          ...
          added.setAttribute('data-dsh-stream-head', '');
    ...
    const runScripts = descriptors => {
      for (let index = scheduledScripts; index < descriptors.length; index += 1) {
        ...
        scriptQueue = scriptQueue.then(() => new Promise(resolve => {
          const pending = document.querySelector('script[data-dsh-script-index="' + index + '"]');
          if (!pending || pending.hasAttribute('data-dsh-script-executed')) {
            resolve();
            return;
          }
          const script = document.createElement('script');
          descriptor.attributes.forEach(([name, value]) => script.setAttribute(name, value));
          script.setAttribute('data-dsh-script-index', String(index));
          script.setAttribute('data-dsh-script-executed', '');
          if (pending.hasAttribute('data-dsh-stream-head')) script.setAttribute('data-dsh-stream-head', '');
    ...
    addEventListener('message', event => {
      ...
      if (data?.type === 'dsh-html-theme') {
        applyTheme(data.theme);
        return;
      }
      if (data?.type !== 'dsh-html-stream' || data.id !== id || typeof data.html !== 'string') return;
...
function MarkdownHtmlArtifact({
  ...
  const sendStreamingHtml = useCallback(() => {
    ...
    streamFrameRef.current?.contentWindow?.postMessage(
      { type: 'dsh-html-stream', id: resizeId, html: code, complete: !isIncomplete, scriptCount },
      '*',
    )
  ...
  const sendTheme = useCallback(() => {
    const message = { type: 'dsh-html-theme', theme }
  ...
  useEffect(() => {
    const handleFrameMessage = (event: MessageEvent) => {
      const data = event.data
      if (data?.type !== 'dsh-html-interaction' && data?.type !== 'dsh-html-resize') return
      ...
      if (data.type === 'dsh-html-interaction') {
