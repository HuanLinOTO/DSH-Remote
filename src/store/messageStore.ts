...
class MessageStore {
  ...
  // ============================================
  // DSH chat feed (dsh bridge — ChatItem 源于 event-reducer)
  // ============================================

  /** Whole-session replacement from the dsh reducer (ascending order). */
  setDshMessages(
    sessionId: string,
    messages: Message[],
    meta: {
    ...
    state.isStreaming = meta.isStreaming
    state.loadState = meta.loadState ?? 'loaded'
    state.loadError = undefined
    if (meta.hasMoreHistory !== undefined) state.hasMoreHistory = meta.hasMoreHistory
    if (meta.title !== undefined) state.title = meta.title
    state.isStale = false
    this.notify([sessionId])
  }

  /** Prepend an older history page (dedupe by id) and update hasMore. */
  prependDshMessages(sessionId: string, older: Message[], hasMore: boolean) {
    const state = this.sessions.get(sessionId)
    if (!state) return
    const existingIds = new Set(state.messages.map(m => m.info.id))
