...
describe('serverStore health check', () => {
  ...
  it('marks a valid server health response as online', async () => {
  ...
  it('rejects JSON that is not a server health response', async () => {
    ...
    expect(health.status).toBe('error')
    expect(health.error).toBe('Not a DSH Remote server')
  })
