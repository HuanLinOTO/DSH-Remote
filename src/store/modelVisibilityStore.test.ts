...
function readHiddenKeys(): string[] {
  // 测试环境的 localStorage 可能是 Node 内置实现（不支持 length/key(i) 枚举），
  // 直接按 per-server 约定读取（serverStore 默认 active server id 为 'local'）
  const raw = localStorage.getItem('srv:local:hidden-model-keys')
  if (!raw) return []
