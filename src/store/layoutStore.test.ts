...
describe('LayoutStore panel and terminal layout', () => {
  ...
  it('persists terminal snapshots and restores them on the next sync', () => {
    ...
    const persisted = JSON.parse(localStorage.getItem('dsh-terminal-layout') ?? 'null')
  ...
  it('persists terminal clipboard interaction preferences', () => {
    ...
    expect(localStorage.getItem('dsh-terminal-copy-on-select')).toBe('true')
    expect(localStorage.getItem('dsh-terminal-right-click-paste')).toBe('true')
