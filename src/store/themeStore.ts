...
const STYLE_ID_THEME = 'dsh-theme-vars'
const STYLE_ID_FONT_SCALE = 'dsh-font-scale'
const STYLE_ID_CUSTOM = 'dsh-custom-css'
...
class ThemeStore {
  ...
  applyTheme() {
    ...
    requestAnimationFrame(() => {
      ...
      // 兼容旧 OpenCodeUI Android 包装器的桥接名：优先读 DSH 名，回退旧名
      const bridges = window as unknown as {
        __dsh_android?: { setSystemBars?: (mode: string, bg: string) => void }
        __opencode_android?: { setSystemBars?: (mode: string, bg: string) => void }
      }
      const androidBridge = bridges.__dsh_android ?? bridges.__opencode_android
