...
const STORAGE_KEY = 'dsh-servers'
const ACTIVE_SERVER_KEY = 'dsh-active-server'
export const LOCAL_SERVER_ID = 'local'

/**
 * Server Store
 * 管理多个 DSH Remote 服务器配置
 */
class ServerStore {
      ...
      if (response.ok) {
        ...
        if (!contentType.includes('application/json')) {
          const health: ServerHealth = {
            ...
            error: contentType.includes('text/html')
              ? 'Server returned HTML instead of server health JSON. Check the URL path.'
              : 'Server did not return health JSON',
            details,
          }
        ...
        try {
          ...
          const health: ServerHealth = {
            ...
            latency,
            lastCheck: Date.now(),
            error: 'Invalid server health JSON',
            details,
          }
          return commitHealth(health)
        }
