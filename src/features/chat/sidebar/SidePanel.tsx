...
export function SidePanel({
  ...
  // Host 工作区（dsh 单主机模式）是侧边栏分组的第一等来源，对齐 DSH Web 的
  // 按工作区分组：主机工作区顺序在前，本地另存的目录随后。多服务器模式的目录
  // 仍走 per-server 存储，不与 Host 工作区混合。
  const dshWorkspaces = useDshStore(state => state.workspaces)
  const directoryRows = useMemo<SavedDirectory[]>(() => {
    if (multiServerConfig.enabled || dshWorkspaces.length === 0) return savedDirectories
    const hostRows: SavedDirectory[] = dshWorkspaces.map(workspace => ({
      path: workspace.path,
      name: workspace.title.trim().length > 0 ? workspace.title : getDirectoryName(workspace.path) || workspace.path,
      addedAt: Date.parse(workspace.createdAt) || 0,
    }))
    const localOnly = savedDirectories.filter(
      directory => !hostRows.some(row => isSameDirectory(row.path, directory.path)),
    )
    return [...hostRows, ...localOnly]
  }, [multiServerConfig.enabled, dshWorkspaces, savedDirectories])
  const catalogDirectories = useMemo(
    ...
    [directoryRows, currentDirectory],
  )
  ...
  const [globalFolderIndex, setGlobalFolderIndex] = useState<number>(() => {
    const saved = localStorage.getItem('dsh-sidebar-global-folder-index')
  ...
  // 选中工作区（目录）时，扁平会话列表只显示该目录的会话；未选中（全局）显示全部。
  // sessionLookup / busy 解析仍用全量 sessions，不受此过滤影响。
  const scopedSessions = useMemo(() => {
    if (!currentDirectory) return sessions
    return sessions.filter(
      session => session.directory.length > 0 && isSameDirectory(session.directory, currentDirectory),
    )
  }, [sessions, currentDirectory])

  const orderedSessions = useMemo(() => {
    const scopedSet = new Set(scopedSessions.map(s => s.id))
    ...
    const pinned = pinnedEntries
      ...
      .filter((session): session is ApiSession => session !== undefined && scopedSet.has(session.id))
    const rest = scopedSessions.filter(s => !pinnedSet.has(s.id))
    return [...pinned, ...rest]
  }, [scopedSessions, pinnedEntries, sessionLookup])
  const pinnedDividerAfterIds = useMemo(() => {
    const lastPinned = pinnedEntries
      .map(entry => sessionLookup.get(entry.sessionId))
      .filter((session): session is ApiSession => session !== undefined && scopedSessions.some(s => s.id === session.id))
      .at(-1)
    if (!lastPinned) return undefined
    const pinnedSet = new Set(pinnedEntries.map(e => e.sessionId))
    return scopedSessions.some(s => !pinnedSet.has(s.id)) ? new Set([lastPinned.id]) : undefined
  }, [scopedSessions, pinnedEntries, sessionLookup])
  ...
  // 扁平列表分支专用：置顶也要落在当前工作区范围内才显示
  const scopedResolvedPinnedSessions = useMemo(
    () => {
      if (!currentDirectory) return resolvedPinnedSessions
      return resolvedPinnedSessions.filter(
        session => session.directory.length > 0 && isSameDirectory(session.directory, currentDirectory),
      )
    },
    [resolvedPinnedSessions, currentDirectory],
  )
  ...
  const folderProjectGroups = useMemo<ProjectItem[]>(() => {
    return buildProjectGroups(directoryRows)
  }, [buildProjectGroups, directoryRows])
  ...
  const selectorProjectGroups = useMemo<ProjectItem[]>(() => {
    ...
    const sortedDirectories = [...directoryRows].sort((a, b) => {
  ...
  }, [multiServerConfig.enabled, focusedServerWorkspaces, buildProjectGroups, recentProjects, directoryRows])
    ...
    (draggedId: string, targetId: string) => {
      ...
      if (draggedIsGlobal) {
        ...
        if (targetIdx !== globalFolderIndex) {
          ...
          localStorage.setItem('dsh-sidebar-global-folder-index', String(targetIdx))
        }
        return
      }
      ...
      if (targetIsGlobal) {
        ...
        if (draggedIdx !== globalFolderIndex) {
          ...
          localStorage.setItem('dsh-sidebar-global-folder-index', String(draggedIdx))
        }
        return
      }
            ...
            >
              {multiServerConfig.enabled ? (
                ...
                <SessionList
                  ...
                  unavailablePinnedEntries={unavailablePinnedEntries}
                  availablePinnedCount={scopedResolvedPinnedSessions.length}
