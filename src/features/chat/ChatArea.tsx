...
import { useStreamingSteady } from './virtual/useStreamingSteady'
...
/** session busy 消抖窗口：需长于「发送 → 新 user 入列」竞态，远短于工具间隙/重连重折 */
const STREAMING_STEADY_DELAY_MS = 500
...
interface RowProps {
  ...
  onEntryGrowComplete?: (messageId: string) => void
  /** 展开/折叠状态键的命名空间（session id）：head 壳等固定 key 不能跨会话串状态 */
  disclosureNamespace: string
}
...
const VirtualRow = memo(
            ...
            <div className="flex justify-start">
              <div className="message-renderer-shell min-w-0 group w-full flex flex-col gap-2">
                <ProcessCollapseBlock
                  stateKey={`turn-process:${disclosureNamespace}:${item.userMessageId ?? item.key}`}
  ...
  (prev, next) =>
    ...
    prev.disclosureNamespace === next.disclosureNamespace,
)
...
export const ChatArea = memo(
  forwardRef<ChatAreaHandle, ChatAreaProps>(
    (
      ...
      // session busy 消抖：工具循环间隙 / 重连重折期间，最后一个有内容的回合
      // 保持 Working 不整壳收起（见 buildProcessTimeline 的 sessionRunningSteady）
      const streamingSteady = useStreamingSteady(isStreaming, STREAMING_STEADY_DELAY_MS)
      ...
      const timeline = useMemo<ProcessTimelineItem[]>(() => {
        const next = !processCollapseEnabled
          ...
          : buildProcessTimeline(visibleMessages, {
              ...
              sessionRunningSteady: streamingSteady,
            })
        const previous =
              ...
              {items.map(item => {
                ...
                return (
                  <VirtualRow
                    ...
                    disclosureNamespace={sessionId ?? 'session'}
                  />
                )
              })}
            </div>
