...
describe('buildProcessTimeline', () => {
  ...
  it('collapses a leading orphan segment (no anchor user in window) into a synthetic shell', () => {
    // 懒加载历史窗口从回合中段开始：锚点 user 在更早的页
    const processOnly = createAssistantMessage(
      'assistant-1',
      [createToolPart('tool-1', 'assistant-1')],
      1001,
    ...
    const messages = [processOnly, finalAnswer]
    const timeline = buildProcessTimeline(messages, {
      turnDurationMap: new Map(),
      sessionIsStreaming: false,
      messageHasProcess: hasProcess,
  ...
  it('keeps the head shell Working while the orphan segment is still live', () => {
    const live = createAssistantMessage('assistant-1', [createToolPart('tool-1', 'assistant-1')], 1001)
    live.isStreaming = true
    const timeline = buildProcessTimeline([live], {
      turnDurationMap: new Map(),
      sessionIsStreaming: true,
      messageHasProcess: hasProcess,
      messageHasFinal: hasFinal,
    ...
    expect(timeline).toHaveLength(1)
    const shell = timeline[0]
    expect(shell).toMatchObject({ kind: 'process-shell', key: HEAD_SHELL_KEY, isActive: true })
    if (shell?.kind !== 'process-shell') return
    expect(shell.durationMs).toBeUndefined()
    expect(shell.children.map(c => [c.message.info.id, c.processContentScope])).toEqual([
      ['assistant-1', 'process'],
    ])
    expect(shell.finalMessage).toBeUndefined()
  })

  it('merges the head segment into the anchored turn once the anchor user loads', () => {
    // 往上翻页载入锚点 user：head 壳消失，续段并入 user-1 的壳
    const processOnly = createAssistantMessage(
      'assistant-1',
      [createToolPart('tool-1', 'assistant-1')],
      1001,
    ...
    const messages = [createUserMessage('user-1', 1000), processOnly, finalAnswer]
    const timeline = buildProcessTimeline(messages, {
      turnDurationMap: buildTurnDurationMap(messages, messages),
      sessionIsStreaming: false,
      messageHasProcess: hasProcess,
      messageHasFinal: hasFinal,
    })

    expect(timeline.some(i => i.kind === 'process-shell' && i.key === HEAD_SHELL_KEY)).toBe(false)
    const shell = timeline.find(i => i.kind === 'process-shell')
    if (shell?.kind !== 'process-shell') throw new Error('expected anchored shell')
    expect(shell.key).toBe('process-shell:user-1')
    expect(shell.children.map(c => c.message.info.id)).toEqual(['assistant-1', 'assistant-2'])
  })

  it('falls back to a flat row for a pure-final head orphan', () => {
    const plain = createAssistantMessage(
      'assistant-1',
      [
        {
    ...
    const timeline = buildProcessTimeline([plain], {
      turnDurationMap: new Map(),
      sessionIsStreaming: false,
      messageHasProcess: hasProcess,
      messageHasFinal: hasFinal,
    ...
    expect(timeline[0]).toMatchObject({ kind: 'message', key: 'assistant-1' })
  })

  it('keeps the last content turn Working through tool-loop gaps while the session is busy', () => {
    // 工具间隙：tool/result 已落、下一条 assistant 未到——消息级全 completed，
    // 但会话仍在跑（sessionRunningSteady）→ 最后一个有内容的回合不得整壳收起
    const messages = [
      createUserMessage('user-1', 1000),
      createAssistantMessage('assistant-1', [createToolPart('tool-1', 'assistant-1')], 1001, 1200),
    ]
    const timeline = buildProcessTimeline(messages, {
      turnDurationMap: new Map(),
      sessionIsStreaming: true,
      messageHasProcess: hasProcess,
      messageHasFinal: hasFinal,
      sessionRunningSteady: true,
    })

    const shell = timeline.find(item => item.kind === 'process-shell')
    if (shell?.kind !== 'process-shell') throw new Error('expected shell')
    expect(shell.isActive).toBe(true)
    expect(shell.durationMs).toBeUndefined()
  })

  it('still settles the last turn when the busy signal is not steady yet', () => {
    // 发送瞬间竞态：busy 刚翻转、还没过消抖窗口 → 不保活（旧回合不闪回 Working）
    const messages = [
      createUserMessage('user-1', 1000),
      createAssistantMessage('assistant-1', [createToolPart('tool-1', 'assistant-1')], 1001, 1200),
    ]
  ...
  it('does not reopen an earlier settled turn when a later empty turn is pending', () => {
    // 新 user 已入列：旧回合非最后回合，保活不作用它；空回合挂 Working
    const messages = [
      createUserMessage('user-1', 1000),
      createAssistantMessage('assistant-1', [createToolPart('tool-1', 'assistant-1')], 1001, 1200),
      createUserMessage('user-2', 2000),
    ...
    const shells = timeline.filter(item => item.kind === 'process-shell')
    expect(shells).toHaveLength(2)
    expect(shells[0]).toMatchObject({ isActive: false, userMessageId: 'user-1' })
    expect(shells[1]).toMatchObject({ isActive: true, userMessageId: 'user-2', children: [] })
  })

  it('keeps the head continuation shell Working after a mid-turn reconnect', () => {
    // 重连后历史重折：整窗无锚点 user，消息瞬时全 completed，但会话仍在跑
    ...
    const shell = timeline.find(item => item.kind === 'process-shell')
    if (shell?.kind !== 'process-shell') throw new Error('expected head shell')
    expect(shell.key).toBe(HEAD_SHELL_KEY)
    expect(shell.isActive).toBe(true)
  })
})
