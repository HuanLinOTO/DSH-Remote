...
/** 页首续段壳的固定 key：历史窗口从回合中段开始、锚点 user 未加载时的合成壳 */
export const HEAD_SHELL_KEY = 'process-shell:head'

type ProcessShellInput = {
  key: string
  userMessageId: string | null
  startedAt?: number
  isActive: boolean
...
/**
 * 一个回合（或页首续段）的过程壳；无可折内容时退化为平铺行。
 * 进行中：全部进壳（末位仅 process）；结束：中间全进 + 末尾仅 process + 壳外 final。
 */
function buildProcessShellItems(input: ProcessShellInput): ProcessTimelineItem[] {
  const { assistants, isActive } = input
  ...
  const durationMs = isActive
    ? undefined
    : resolveTurnDurationMs(assistants, input.startedAt, input.turnDurationMap)
  ...
  const finalOutside =
    !isActive && finalAssistant && input.messageHasFinal(finalAssistant) ? finalAssistant : undefined

  // 进行中或有过程内容 → 挂壳
  // 更晚的空 pending 回合：isActive=false 且无 children → 不挂壳（只显示 user）
  // 纯 final 正文、无过程 → 直接平铺
  if (children.length > 0 || isActive) {
    return [
      {
        kind: 'process-shell',
        key: input.key,
  ...
  if (finalOutside) {
    return [{ kind: 'message', key: finalOutside.info.id, message: finalOutside }]
  }
  return assistants.map(m => ({ kind: 'message' as const, key: m.info.id, message: m }))
}
 ...
 *
 * 页首续段：懒加载历史窗口可能从回合中段开始（锚点 user 在更早页），
 * 这部分 assistant 同样收进一个无 user 的合成壳（HEAD_SHELL_KEY），结束后折叠；
 * 往上翻页载入锚点后自然并入对应回合的壳。
 */
export function buildProcessTimeline(
  ...
  options: {
    ...
    /** session busy 消抖信号（持续 streaming 超过消抖窗口）：保活最后一个有内容的回合 */
    sessionRunningSteady?: boolean
  },
): ProcessTimelineItem[] {
  const {
    turnDurationMap,
  ...
  // 页首无 user 锚点的 assistant 续段（历史懒加载窗口从中段开始时出现）
  const headOrphans: Message[] = []
  ...
  // 页首续段先入列（它在窗口最上方），同样挂壳：结束后折叠中间过程
  if (headOrphans.length > 0) {
    items.push(
      ...buildProcessShellItems({
        key: HEAD_SHELL_KEY,
        userMessageId: null,
        startedAt: headOrphans[0].info.time.created,
        isActive: headOrphans.some(assistantHasLiveWork)
          || (sessionRunningSteady && turns.length === 0),
        assistants: headOrphans,
        turnDurationMap,
        messageHasProcess,
        messageHasFinal,
      }),
  ...
  const laterHasAssistant = (fromIndex: number) => {
    for (let j = fromIndex + 1; j < turns.length; j++) {
      if (turns[j].assistants.length > 0) return true
    }
    return false
  }
  ...
  const laterHasLive = (fromIndex: number) => {
    for (let j = fromIndex + 1; j < turns.length; j++) {
      if (turns[j].assistants.some(assistantHasLiveWork)) return true
    }
    return false
  }
  ...
  const isTurnSettled = (turn: TurnBag, index: number): boolean => {
    ...
    // 会话仍在跑（busy 已过消抖窗口）：最后一个有内容的回合保持 Working，
    // 覆盖消息级 live 检测的两类空窗——工具循环间隙（tool/result 已到、
    // 下一条 assistant 未到）与重连后历史重折（消息瞬时全 completed）。
    // 只作用于最后一个回合：发送瞬间 busy 早于新 user 入列时，旧回合不会闪回。
    if (sessionRunningSteady && index === turns.length - 1 && assistants.length > 0) {
      return false
    }
  ...
  for (let i = 0; i < turns.length; i++) {
    if (isTurnPending(turns[i], i)) {
      activeUserId = turns[i].user.info.id
      break
    }
  }
  ...
  for (const turn of turns) {
    ...
    items.push(
      ...buildProcessShellItems({
        key: `process-shell:${turn.user.info.id}`,
        userMessageId: turn.user.info.id,
        startedAt: turn.user.info.time.created,
        isActive: activeUserId != null && turn.user.info.id === activeUserId,
