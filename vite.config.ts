...
export default defineConfig({
  ...
  server: {
    ...
    watch: {
      // Cargo 构建产物目录不能被 watch（Windows 下 cargo 写 .exe 时会锁定文件，导致 EBUSY 崩溃）
      // 外部工具（编辑器/代理进程）常用原子写：<file>.<pid>.<uuid>.tmpdir/<file>.tmp，
      // fs.watch 对这类 tmp 文件同样会抛 EBUSY 并杀死整个 dev server，一并忽略。
